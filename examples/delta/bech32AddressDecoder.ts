...
const bech32Decoder = (item: string) => {
  const bech32Val = item.length === 45 ? item.split("D-").join("") : item
...
const main = async (): Promise<any> => {
  //can be used for O and A addresses too
  const address: string = "D-dione1hwgqh0s6yrdy636xv6me5haxecfx99enh5278a"
