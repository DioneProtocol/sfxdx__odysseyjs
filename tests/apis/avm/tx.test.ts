...
describe("Transactions", (): void => {
  ...
  beforeAll(async (): Promise<void> => {
    ...
    dioneAssetID = await result
  })

  beforeEach((): void => {
    set = new UTXOSet()
    keymgr1 = new KeyChain(odyssey.getHRP(), alias)
    ...
    amount = ONEDIONE.mul(new BN(amnt))
  ...
  test("Creation UnsignedTx Check Amount", (): void => {
    expect((): void => {
      set.buildBaseTx(
        netid,
        blockchainID,
        ONEDIONE.mul(new BN(amnt * 10000)),
        assetID,
        addrs3,
        addrs1,
        addrs1
