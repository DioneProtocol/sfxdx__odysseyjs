...
export const SelectInputClass = (inputID: number, ...args: any[]): Input => {
  if (inputID === DELTAConstants.SECPINPUTID) {
...
export class TransferableInput extends StandardTransferableInput {
  ...
  getCost = (): number => {
    ...
    return numSigs * Defaults.network[1].D.costPerSignature
  }
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    ...
    this.assetID = bintools.copyFrom(
      bytes,
      offset,
      offset + DELTAConstants.ASSETIDLEN
    )
    offset += 32
...
export class SECPTransferInput extends AmountInput {
  ...
  protected _typeID = DELTAConstants.SECPINPUTID
   ...
   */
  getInputID(): number {
    return DELTAConstants.SECPINPUTID
  }

  getCredentialID = (): number => DELTAConstants.SECPCREDENTIAL
...
export class DELTAInput extends DELTAOutput {
  ...
  /**
   * Returns a {@link https://github.com/feross/buffer|Buffer} representation of the [[DELTAOutput]].
   */
  toBuffer(): Buffer {
  ...
  /**
   * Decodes the [[DELTAInput]] as a {@link https://github.com/feross/buffer|Buffer} and returns the size.
   *
  ...
  /**
   * Returns a base-58 representation of the [[DELTAInput]].
   */
  toString(): string {
  ...
  create(...args: any[]): this {
    return new DELTAInput(...args) as this
  }

  clone(): this {
    const newDELTAInput: DELTAInput = this.create()
    newDELTAInput.fromBuffer(this.toBuffer())
    return newDELTAInput as this
  }

  /**
   * An [[DELTAInput]] class which contains address, amount, assetID, nonce.
   *
   * @param address is the DELTA address from which to transfer funds.
   * @param amount is the amount of the asset to be transferred (specified in nDIONE for DIONE and the smallest denomination for all other assets).
