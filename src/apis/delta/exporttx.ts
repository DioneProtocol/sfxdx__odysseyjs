...
export class ExportTx extends DELTABaseTx {
  protected _typeName = "ExportTx"
  protected _typeID = DELTAConstants.EXPORTTX
  ...
  /**
   * Returns the inputs as an array of [[DELTAInputs]]
   */
  getInputs(): DELTAInput[] {
    return this.inputs
  }

  /**
   * Returns the outs as an array of [[DELTAOutputs]]
   */
  ...
  toBuffer(): Buffer {
    ...
    this.inputs.forEach((importIn: DELTAInput) => {
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    ...
    for (let i: number = 0; i < numInputs; i++) {
      const anIn: DELTAInput = new DELTAInput()
  ...
  sign(msg: Buffer, kc: KeyChain): Credential[] {
    ...
    this.inputs.forEach((input: DELTAInput) => {
   ...
   * @param inputs Optional array of the [[DELTAInputs]]s
   * @param exportedOutputs Optional array of the [[DELTAOutputs]]s
   */
  constructor(
    networkID: number = undefined,
    blockchainID: Buffer = Buffer.alloc(32, 16),
    destinationChain: Buffer = Buffer.alloc(32, 16),
    inputs: DELTAInput[] = undefined,
    exportedOutputs: TransferableOutput[] = undefined
  ) {
    super(networkID, blockchainID)
    this.destinationChain = destinationChain
    if (typeof inputs !== "undefined" && Array.isArray(inputs)) {
      inputs.forEach((input: DELTAInput) => {
        if (!(input instanceof DELTAInput)) {
          throw new DELTAInputError(
            "Error - ExportTx.constructor: invalid DELTAInput in array parameter 'inputs'"
          )
        }
      })
      if (inputs.length > 1) {
      ...
      exportedOutputs.forEach((exportedOutput: TransferableOutput) => {
        if (!(exportedOutput instanceof TransferableOutput)) {
          throw new TransferableOutputError(
            "Error - ExportTx.constructor: TransferableOutput DELTAInput in array parameter 'exportedOutputs'"
          )
        }
      })
