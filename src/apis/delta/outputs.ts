...
export const SelectOutputClass = (outputID: number, ...args: any[]): Output => {
  if (outputID == DELTAConstants.SECPXFEROUTPUTID) {
...
export class SECPTransferOutput extends AmountOutput {
  ...
  protected _typeID = DELTAConstants.SECPXFEROUTPUTID
...
export class DELTAOutput {
  ...
  /**
   * Decodes the [[DELTAOutput]] as a {@link https://github.com/feross/buffer|Buffer} and returns the size.
   */
  fromBuffer(bytes: Buffer, offset: number = 0): number {
  ...
  /**
   * Returns a base-58 representation of the [[DELTAOutput]].
   */
  toString(): string {
  ...
  create(...args: any[]): this {
    return new DELTAOutput(...args) as this
  }

  clone(): this {
    const newDELTAOutput: DELTAOutput = this.create()
    newDELTAOutput.fromBuffer(this.toBuffer())
    return newDELTAOutput as this
  }

  /**
   * An [[DELTAOutput]] class which contains address, amount, and assetID.
   *
