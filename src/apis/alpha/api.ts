...
import { Defaults, PrimaryAssetAlias, ONEDIONE } from "../../utils/constants"
...
/**
 * Class for interacting with a node endpoint that is using the ALPHA.
 *
 * @category RPCAPIs
 *
 ...
 */
export class ALPHAAPI extends JRPCAPI {
  /**
   * @ignore
   */
  ...
  refreshBlockchainID = (blockchainID: string = undefined): boolean => {
    ...
    if (
      ...
      this.blockchainID = Defaults.network[`${netid}`].A.blockchainID //default to A-Chain
      return true
    }
  ...
  getDIONEAssetID = async (refresh: boolean = false): Promise<Buffer> => {
    if (typeof this.DIONEAssetID === "undefined" || refresh) {
      const asset: GetDIONEAssetIDParams = await this.getAssetDescription(
        PrimaryAssetAlias
      )
  ...
  getDefaultTxFee = (): BN => {
    return this.core.getNetworkID() in Defaults.network
      ? new BN(Defaults.network[this.core.getNetworkID()]["A"]["txFee"])
      : new BN(0)
  }
  ...
  getDefaultCreationTxFee = (): BN => {
    return this.core.getNetworkID() in Defaults.network
      ? new BN(Defaults.network[this.core.getNetworkID()]["A"]["creationTxFee"])
      : new BN(0)
  }
  ...
  getDefaultMintTxFee = (): BN => {
    return this.core.getNetworkID() in Defaults.network
      ? new BN(Defaults.network[this.core.getNetworkID()]["A"]["mintTxFee"])
      : new BN(0)
  }
  ...
  getBalance = async (
    ...
    if (typeof this.parseAddress(address) === "undefined") {
      ...
      throw new AddressError(
        "Error - ALPHAAPI.getBalance: Invalid address format"
      )
    }
   ...
   *
   * @param username The user paying the transaction fee (in $DIONE) for asset creation
   * @param password The password for the user paying the transaction fee (in $DIONE) for asset creation
   ...
   * [
   *   {
   *     "address": "A-dione1kj06lhgx84h39snsljcey3tpc046ze68mek3g5",
   *     "amount": 10000
   *   },
   *   {
   *     "address": "A-dione1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr",
   *     "amount": 50000
   *   }
   * ]
   * ```
   ...
   *    {
   *      "minters":[
   *        "A-dione1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr"
   *      ],
   *      "threshold": 1
   *     },
   *     {
   *      "minters": [
   *        "A-dione1am4w6hfrvmh3akduzkjthrtgtqafalce6an8cr",
   *        "A-dione1kj06lhgx84h39snsljcey3tpc046ze68mek3g5",
   *        "A-dione1yell3e4nln0m39cfpdhgqprsd87jkh4qnakklx"
   *      ],
   *      "threshold": 2
   *     }
   * ]
  ...
  createNFTAsset = async (
    ...
    if (typeof changeAddr !== "undefined") {
      if (typeof this.parseAddress(changeAddr) === "undefined") {
        /* istanbul ignore next */
        throw new AddressError(
          "Error - ALPHAAPI.createNFTAsset: Invalid address format"
        )
      }
  ...
  /**
   * Mint non-fungible tokens which were created with ALPHAAPI.createNFTAsset
   *
   * @param username The user paying the transaction fee (in $DIONE) for asset creation
   * @param password The password for the user paying the transaction fee (in $DIONE) for asset creation
   * @param from Optional. An array of addresses managed by the node's keystore for this blockchain which will fund this transaction
   ...
   * @param to Address on A-Chain of the account to which this NFT is being sent
  ...
  /**
   * Send NFT from one account to another on A-Chain
   *
   * @param username The user paying the transaction fee (in $DIONE) for asset creation
   * @param password The password for the user paying the transaction fee (in $DIONE) for asset creation
   * @param from Optional. An array of addresses managed by the node's keystore for this blockchain which will fund this transaction
  ...
  /**
   * Send ANT (Odyssey Native Token) assets including DIONE from the A-Chain to an account on the O-Chain or D-Chain.
   *
   * After calling this method, you must call the O-Chain's `import` or the D-Chain’s `import` method to complete the transfer.
   *
   * @param username The Keystore user that controls the O-Chain or D-Chain account specified in `to`
   * @param password The password of the Keystore user
   * @param to The account on the O-Chain or D-Chain to send the asset to.
  ...
  /**
   * Send ANT (Odyssey Native Token) assets including DIONE from an account on the O-Chain or D-Chain to an address on the A-Chain. This transaction
   ...
   * @param sourceChain The chainID where the funds are coming from. Ex: "D"
   *
  ...
  getAllBalances = async (address: string): Promise<object[]> => {
    if (typeof this.parseAddress(address) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError(
        "Error - ALPHAAPI.getAllBalances: Invalid address format"
      )
    }
  ...
  buildBaseTx = async (
    ...
    const feeAssetID: Buffer = await this.getDIONEAssetID()
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      ...
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildBaseTx:Failed Goose Egg Check"
      )
    }
  ...
  buildNFTTransferTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildNFTTransferTx:Failed Goose Egg Check"
      )
    }
  ...
  buildImportTx = async (
    ...
    if (typeof sourceChain === "undefined") {
      throw new ChainIdError(
        "Error - ALPHAAPI.buildImportTx: Source ChainID is undefined."
      )
      ...
      throw new ChainIdError(
        "Error - ALPHAAPI.buildImportTx: Invalid destinationChain type: " +
          typeof sourceChain
      )
    }
    ...
    if (atomics.length === 0) {
      throw new NoAtomicUTXOsError(
        "Error - ALPHAAPI.buildImportTx: No atomic UTXOs to import from " +
          srdChain +
          " using addresses: " +
          ownerAddresses.join(", ")
      )
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildImportTx:Failed Goose Egg Check"
      )
    }
   ...
   * @param locktime Optional. The locktime field created in the resulting outputs
   * @param threshold Optional. The number of signatures required to spend the funds in the resultant UTXO
   * @param assetID Optional. The assetID of the asset to send. Defaults to DIONE assetID.
   * Regardless of the asset which you"re exporting, all fees are paid in DIONE.
   *
  ...
  buildExportTx = async (
    ...
    if (Object.keys(prefixes).length !== 1) {
      throw new AddressError(
        "Error - ALPHAAPI.buildExportTx: To addresses must have the same chainID prefix."
      )
    }

    if (typeof destinationChain === "undefined") {
      throw new ChainIdError(
        "Error - ALPHAAPI.buildExportTx: Destination ChainID is undefined."
      )
    } else if (typeof destinationChain === "string") {
      destinationChain = bintools.cb58Decode(destinationChain) //
    } else if (!(destinationChain instanceof Buffer)) {
      throw new ChainIdError(
        "Error - ALPHAAPI.buildExportTx: Invalid destinationChain type: " +
          typeof destinationChain
      )
    }
    if (destinationChain.length !== 32) {
      throw new ChainIdError(
        "Error - ALPHAAPI.buildExportTx: Destination ChainID must be 32 bytes in length."
      )
    }
    ...
    if (typeof assetID === "undefined") {
      assetID = bintools.cb58Encode(dioneAssetID)
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildExportTx:Failed Goose Egg Check"
      )
    }
   ...
   * @param denomination Number for the denomination which is 10^D. D must be >= 0 and <= 32. Ex: $1 DIONE = 10^9 $nDIONE
  ...
  buildCreateAssetTx = async (
    ...
    if (symbol.length > ALPHAConstants.SYMBOLMAXLEN) {
      throw new SymbolError(
        "Error - ALPHAAPI.buildCreateAssetTx: Symbols may not exceed length of " +
          ALPHAConstants.SYMBOLMAXLEN
      )
    }
    if (name.length > ALPHAConstants.ASSETNAMELEN) {
      throw new NameError(
        "Error - ALPHAAPI.buildCreateAssetTx: Names may not exceed length of " +
          ALPHAConstants.ASSETNAMELEN
      )
    }
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, fee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildCreateAssetTx:Failed Goose Egg Check"
      )
    }
  ...
  buildSECPMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildSECPMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
   ...
   *      {
   *          "minters":[
   *              "A-dione1ghstjukrtw8935lryqtnh643xe9a94u3tc75c7"
   *          ],
   *          "threshold": 1
   *      },
   *      {
   *          "minters": [
   *              "A-dione1yell3e4nln0m39cfpdhgqprsd87jkh4qnakklx",
   *              "A-dione1k4nr26c80jaquzm9369j5a4shmwcjn0vmemcjz",
   *              "A-dione1ztkzsrjnkn0cek5ryvhqswdtcg23nhge3nnr5e"
   *          ],
   *          "threshold": 2
   *      }
   * ]
  ...
  buildCreateNFTAssetTx = async (
    ...
    if (name.length > ALPHAConstants.ASSETNAMELEN) {
      /* istanbul ignore next */
      throw new NameError(
        "Error - ALPHAAPI.buildCreateNFTAssetTx: Names may not exceed length of " +
          ALPHAConstants.ASSETNAMELEN
      )
    }
    if (symbol.length > ALPHAConstants.SYMBOLMAXLEN) {
      /* istanbul ignore next */
      throw new SymbolError(
        "Error - ALPHAAPI.buildCreateNFTAssetTx: Symbols may not exceed length of " +
          ALPHAConstants.SYMBOLMAXLEN
      )
    }
    const networkID: number = this.core.getNetworkID()
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx, creationTxFee))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildCreateNFTAssetTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  buildCreateNFTMintTx = async (
    ...
    if (!(await this.checkGooseEgg(builtUnsignedTx))) {
      /* istanbul ignore next */
      throw new GooseEggCheckError(
        "Error - ALPHAAPI.buildCreateNFTMintTx:Failed Goose Egg Check"
      )
    }
    return builtUnsignedTx
  }
  ...
  issueTx = async (tx: string | Buffer | Tx): Promise<string> => {
    ...
    if (typeof tx === "string") {
      ...
      throw new TransactionError(
        "Error - ALPHAAPI.issueTx: provided tx is not expected type of string, Buffer, or Tx"
      )
    }
  ...
  send = async (
    ...
    if (typeof this.parseAddress(to) === "undefined") {
      /* istanbul ignore next */
      throw new AddressError("Error - ALPHAAPI.send: Invalid address format")
    }
    ...
    sendOutputs.forEach(
      (output: {
        ...
        if (typeof this.parseAddress(output.to) === "undefined") {
          /* istanbul ignore next */
          throw new AddressError(
            "Error - ALPHAAPI.sendMultiple: Invalid address format"
          )
        }
      ...
      for (let i: number = 0; i < addresses.length; i++) {
        if (typeof addresses[`${i}`] === "string") {
          if (
            ...
            /* istanbul ignore next */
            throw new AddressError(
              "Error - ALPHAAPI.${caller}: Invalid address format"
            )
          }
  ...
  /**
   * This class should not be instantiated directly. Instead use the [[Odyssey.addAP`${I}`]] method.
   *
   * @param core A reference to the Odyssey class
   * @param baseURL Defaults to the string "/ext/bc/A" as the path to blockchain's baseURL
   * @param blockchainID The Blockchain"s ID. Defaults to an empty string: ""
   */
  constructor(
    core: OdysseyCore,
    baseURL: string = "/ext/bc/A",
    blockchainID: string = ""
  ) {
    super(core, baseURL)
    this.blockchainID = blockchainID
