...
export class UTXO extends StandardUTXO {
  ...
  create(
    codecID: number = ALPHAConstants.LATESTCODEC,
...
export class UTXOSet extends StandardUTXOSet<UTXO> {
  ...
  buildSECPMintTx = (
    ...
    if (utxo.getOutput().getOutputID() !== ALPHAConstants.SECPMINTOUTPUTID) {
      throw new SECPMintOutputError(
  ...
  buildCreateNFTAssetTx = (
    ...
    for (let i: number = 0; i < minterSets.length; i++) {
      ...
      initialState.addOutput(nftMintOutput, ALPHAConstants.NFTFXID)
    }
