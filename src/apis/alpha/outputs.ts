...
export const SelectOutputClass = (outputid: number, ...args: any[]): Output => {
  if (
    outputid === ALPHAConstants.SECPXFEROUTPUTID ||
    outputid === ALPHAConstants.SECPXFEROUTPUTID_CODECONE
  ) {
    return new SECPTransferOutput(...args)
  } else if (
    outputid === ALPHAConstants.SECPMINTOUTPUTID ||
    outputid === ALPHAConstants.SECPMINTOUTPUTID_CODECONE
  ) {
    return new SECPMintOutput(...args)
  } else if (
    outputid === ALPHAConstants.NFTMINTOUTPUTID ||
    outputid === ALPHAConstants.NFTMINTOUTPUTID_CODECONE
  ) {
    return new NFTMintOutput(...args)
  } else if (
    outputid === ALPHAConstants.NFTXFEROUTPUTID ||
    outputid === ALPHAConstants.NFTXFEROUTPUTID_CODECONE
  ) {
...
export class TransferableOutput extends StandardTransferableOutput {
  ...
  fromBuffer(bytes: Buffer, offset: number = 0): number {
    this.assetID = bintools.copyFrom(
      bytes,
      offset,
      offset + ALPHAConstants.ASSETIDLEN
    )
    offset += ALPHAConstants.ASSETIDLEN
...
export class NFTTransferOutput extends NFTOutput {
  ...
  protected _typeID =
    this._codecID === 0
      ...
      : ALPHAConstants.NFTXFEROUTPUTID_CODECONE
