...
export const SelectOperationClass = (
  ...
  if (
    opid === ALPHAConstants.SECPMINTOPID ||
    opid === ALPHAConstants.SECPMINTOPID_CODECONE
  ) {
    return new SECPMintOperation(...args)
  } else if (
    opid === ALPHAConstants.NFTMINTOPID ||
    opid === ALPHAConstants.NFTMINTOPID_CODECONE
  ) {
    return new NFTMintOperation(...args)
  } else if (
    opid === ALPHAConstants.NFTXFEROPID ||
    opid === ALPHAConstants.NFTXFEROPID_CODECONE
  ) {
...
export class TransferableOperation extends Serializable {
  ...
  constructor(
    ...
    if (
      ...
      assetID.length === ALPHAConstants.ASSETIDLEN &&
...
export class SECPMintOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? ALPHAConstants.SECPMINTOPID
      : ALPHAConstants.SECPMINTOPID_CODECONE
  ...
  getCredentialID(): number {
    if (this._codecID === 0) {
      return ALPHAConstants.SECPCREDENTIAL
    } else if (this._codecID === 1) {
      return ALPHAConstants.SECPCREDENTIAL_CODECONE
    }
  }
...
export class NFTMintOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? ALPHAConstants.NFTMINTOPID
      : ALPHAConstants.NFTMINTOPID_CODECONE
  ...
  getCredentialID = (): number => {
    if (this._codecID === 0) {
      return ALPHAConstants.NFTCREDENTIAL
    } else if (this._codecID === 1) {
      return ALPHAConstants.NFTCREDENTIAL_CODECONE
    }
  }
...
export class NFTTransferOperation extends Operation {
  ...
  protected _typeID =
    this._codecID === 0
      ? ALPHAConstants.NFTXFEROPID
      : ALPHAConstants.NFTXFEROPID_CODECONE
