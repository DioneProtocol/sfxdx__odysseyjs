 ...
 * @module API-ALPHA-InitialStates
 */
...
export class InitialStates extends Serializable {
   ...
   * @param fxid The FxID that will be used for this output, default ALPHAConstants.SECPFXID
   */
  addOutput(out: Output, fxid: number = ALPHAConstants.SECPFXID): void {
    if (!(fxid in this.fxs)) {
      this.fxs[`${fxid}`] = []
    }
